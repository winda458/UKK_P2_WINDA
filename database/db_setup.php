...
try {
    ...
    $pdo->exec("CREATE DATABASE IF NOT EXISTS db_parkir");
    $pdo->exec("USE db_parkir");

    // tb_user
    $pdo->exec("CREATE TABLE IF NOT EXISTS tb_user (
    ...
    $petugas_pass = password_hash('petugas', PASSWORD_DEFAULT);
    $owner_pass = password_hash('owner', PASSWORD_DEFAULT);

    $pdo->exec("INSERT IGNORE INTO tb_user (id_user, nama_lengkap, username, password, role) VALUES 
        (1, 'Administrator', 'admin', '$admin_pass', 'admin'),
        (2, 'Petugas Parkir', 'petugas', '$petugas_pass', 'petugas'),
        (3, 'Owner Parkir', 'owner', '$owner_pass', 'owner')
    ");

    // tb_tarif
    $pdo->exec("CREATE TABLE IF NOT EXISTS tb_tarif (
        id_tarif INT(11) PRIMARY KEY AUTO_INCREMENT,
        jenis_kendaraan ENUM('motor', 'mobil', 'lainnya'),
        tarif_per_jam DECIMAL(10,0)
    )");
    
    $pdo->exec("INSERT IGNORE INTO tb_tarif (id_tarif, jenis_kendaraan, tarif_per_jam) VALUES 
        (1, 'motor', 2000),
        (2, 'mobil', 5000),
        (3, 'lainnya', 10000)
    ");
    ...
    // tb_area_parkir
    $pdo->exec("CREATE TABLE IF NOT EXISTS tb_area_parkir (
    ...
    $pdo->exec("INSERT IGNORE INTO tb_area_parkir (id_area, nama_area, kapasitas, terisi) VALUES 
        (1, 'Area Motor A', 100, 0),
        (2, 'Area Mobil B', 50, 0)
    ");

    // tb_kendaraan
    $pdo->exec("CREATE TABLE IF NOT EXISTS tb_kendaraan (
    ...
    // tb_transaksi
    $pdo->exec("CREATE TABLE IF NOT EXISTS tb_transaksi (
    ...
    // tb_log_aktivitas
    $pdo->exec("CREATE TABLE IF NOT EXISTS tb_log_aktivitas (
    ...
    echo "Database & Tables Setup Successful.";
